# ArcGIS Pro MCP Bridge

ArcGIS Pro MCP Bridge connects MCP-capable AI agents to a live ArcGIS Pro
session through a local ArcGIS Pro add-in and a local MCP server.

Use it when you want an agent to inspect maps, layers, layouts, legends, project
state, visual previews, exports, geoprocessing calls, and guarded ArcPy script
runs without giving the agent unrestricted access to ArcGIS Pro.

## What It Includes

- `ArcGisProBridgeAddIn/` - ArcGIS Pro add-in that runs inside ArcGIS Pro,
  including the Claude dock pane.
- `ArcGisProMcpServer/` - MCP stdio server that exposes ArcGIS Pro tools.
- `ArcGisProBridgeContracts/` - shared request/response contracts.
- `scripts/` - helper scripts for publishing the server and packaging the add-in.
- `arcgis-pro-mcp.config.json` - conservative default bridge configuration.
- `.mcp.json` - development MCP client config that runs the server from source.
- `docs/arcpy-mapmaking-guide.md` - optional ArcPy map-making guidance for agents.

## Features

- A Claude dock pane inside ArcGIS Pro, wired to the running session.
- Project, map, layer, layout, and object-registry inspection.
- Active-view, active-map, and layout-preview image export.
- Guarded layer, map, layout, legend, export, and project-save operations.
- Guarded ArcGIS Pro geoprocessing execution.
- Guarded ArcPy script execution through the open ArcGIS Pro session.
- Dry-run support and confirmation flags for higher-risk actions.
- Optional allowed-root, artifact-directory, timeout, tool-group, and audit-log
  configuration.

## Requirements

- Windows.
- ArcGIS Pro 3.x.
- A .NET SDK matching the ArcGIS Pro release (see below).
- PowerShell.
- Visual Studio with the ArcGIS Pro SDK for the normal add-in build workflow.

The MCP server can be built with the .NET CLI. The ArcGIS Pro add-in can be
compiled with the .NET CLI, but normal Esri add-in packaging is best done with
Visual Studio and the ArcGIS Pro SDK. A PowerShell fallback packaging script is
included.

### ArcGIS Pro version and .NET runtime

The add-in is loaded into the ArcGIS Pro process, so it must target the same .NET
runtime that Pro runs on, and that changed across Pro releases:

| ArcGIS Pro | .NET runtime | Add-in target framework |
| ---------- | ------------ | ----------------------- |
| 3.5, 3.6   | .NET 8       | `net8.0-windows8.0`     |
| 3.7        | .NET 10      | `net10.0-windows`       |

`Directory.Build.props` resolves both the ArcGIS Pro install directory and the
add-in target framework automatically:

- The install directory is read from `HKCU`/`HKLM` `SOFTWARE\ESRI\ArcGISPro`, so
  per-user installs (under `%LOCALAPPDATA%\Programs\ArcGIS\Pro`) work as well as
  per-machine installs under `C:\Program Files`.
- The target framework is read back from Pro's own `ArcGISPro.runtimeconfig.json`.

Override either one if detection fails:

```powershell
dotnet build ArcGISProMcpBridge.sln -p:ArcGISProInstallDir="D:\ArcGIS\Pro\" -p:ArcGISProAddInTargetFramework=net8.0-windows8.0
```

The MCP server itself targets `net8.0` and runs on any newer installed runtime,
so it does not need to track the Pro version.

## Quick Start

Build the solution:

```powershell
dotnet build ArcGISProMcpBridge.sln
```

Publish the MCP server:

```powershell
.\scripts\publish-server.ps1 -Configuration Release
```

Package and register the ArcGIS Pro add-in:

```powershell
.\scripts\package-addin.ps1 -Configuration Debug
```

Restart ArcGIS Pro after installing the add-in.

## Run During Development

Run the MCP server from source:

```powershell
dotnet run --project ArcGisProMcpServer/ArcGisProMcpServer.csproj
```

For development clients, point your MCP client at `.mcp.json`.

For installed use, point your MCP client at the executable generated by:

```powershell
.\scripts\publish-server.ps1 -Configuration Release
```

The publish script writes generated output under `publish/`, which is ignored by
git because it is machine-specific.

## Claude Dock Pane

The add-in can host Claude Code in an ArcGIS Pro dock pane, so the agent sits
beside the map instead of in a separate window. Click **Claude** on the **MCP
Bridge** tab.

The pane runs the real Claude Code CLI under a Windows pseudo console, rendered
with [xterm.js](https://xtermjs.org/) in a web view themed to match ArcGIS Pro.
Everything the CLI does works unchanged: slash commands, plan mode, permission
prompts, `/resume`.

### What the pane sets up for you

- **The MCP server is already connected.** The pane generates an MCP config
  pointing at the published server and passes it to Claude Code with
  `--mcp-config`, so `arcgis-pro` tools are available immediately with no
  `claude mcp add` step. Your own MCP servers are still loaded; set
  `claudePane.strictMcpConfig` to `true` if you want only the generated one.
- **The session starts in your project.** The working directory is the open
  project's home folder, so Claude Code's file tools land on your data.
- **The bridge configuration is shared.** The spawned server inherits the same
  config file and pipe name the add-in itself loaded, so the pane cannot end up
  talking to a differently configured bridge.

### Requirements

Claude Code must be installed and authenticated. The pane looks for it on
`PATH`, then in `%APPDATA%\npm` and `%USERPROFILE%\.local\bin`:

```powershell
npm install -g @anthropic-ai/claude-code
```

The MCP server must be published, and the pane needs to be able to find it. It
looks at `%LOCALAPPDATA%\ArcGISProMcpBridge\server\ArcGisProMcpServer.exe` and
beside the add-in assembly, so either copy the published output there or set
`claudePane.serverPath`:

```powershell
.\scripts\publish-server.ps1 -Configuration Release
```

The pane also needs the Microsoft Edge WebView2 runtime, which ArcGIS Pro
installs as part of its own setup.

If either program cannot be found, the pane says which one and what to do about
it rather than failing silently.

### Keyboard

`Ctrl+C` interrupts as usual, except when text is selected, in which case it
copies. `Ctrl+Shift+C` and `Ctrl+Shift+V` always copy and paste. Right-click
copies a selection if there is one and otherwise pastes.

### Settings

All optional, under `claudePane` in the bridge configuration file:

| Setting | Default | Purpose |
| ------- | ------- | ------- |
| `enabled` | `true` | Turns the ribbon button off. |
| `executablePath` | auto | Full path to the Claude Code executable. |
| `serverPath` | auto | Full path to the published `ArcGisProMcpServer.exe`. |
| `workingDirectory` | project home | Overrides where the session starts. |
| `arguments` | `[]` | Extra arguments appended to the launch. |
| `strictMcpConfig` | `false` | Load only the generated MCP config. |
| `fontFamily` | Cascadia Mono | Terminal font stack. |
| `fontSize` | `13` | Terminal font size in pixels. |
| `lineHeight` | `1.35` | Terminal line height multiplier. |

The pane writes its generated MCP config and unpacked web assets under
`%LOCALAPPDATA%\ArcGISProMcpBridge\pane\`.

## Configuration

The bridge reads configuration from the first available location:

1. `ARCGIS_PRO_MCP_CONFIG`
2. `arcgis-pro-mcp.config.json` in the current working directory
3. `arcgis-pro-mcp.config.json` beside the published server executable
4. `%LOCALAPPDATA%\ArcGISProMcpBridge\config.json`

The included config uses conservative defaults:

- destructive geoprocessing is disabled by default
- feature edits are disabled by default
- save, overwrite, destructive geoprocessing, and script execution actions
  require explicit confirmation flags
- timeouts and enabled tool groups are configurable

## Audit Log

The bridge writes one JSON line per operation to an audit log. Logging is **on by
default**: leaving `auditLogPath` set to `null` does not disable it, it selects
`%LOCALAPPDATA%\ArcGISProMcpBridge\logs\audit.jsonl`. Set the path explicitly, or
override it with `ARCGIS_PRO_MCP_AUDIT_LOG`, to write somewhere else.

Both the MCP server and the ArcGIS Pro add-in append to the log independently, so
each operation is attested twice: once by the process that sent it and once by the
process that executed it.

Each record includes:

- `timestampUtc`, `op`, `group`, `dryRun`, `client`, and elapsed time
- `actor` - the Windows user, domain, machine, and process ID that ran the operation
- `argsSummary` - arguments, with `argsFidelity` recording whether they were
  summarized or captured verbatim
- `targetIds` - the object IDs the operation acted on
- `result` - success or the error code, including operations refused by the
  configured tool groups, confirmations, or destructive-operation policy
- `artifacts` - any exports or previews produced, with their paths

### Argument fidelity

Arguments are summarized by default: strings are truncated at 300 characters and
arrays and objects are reduced to their element counts. Arguments that *are* the
action rather than a handle to one are recorded verbatim instead, up to 20,000
characters. That covers every `geoprocessing.*` and `python.*` operation, plus
argument keys such as `definitionQuery`, `parameters`, `environments`,
`arguments`, `expression`, `sql`, and `whereClause` wherever they appear.

This matters for reconstruction: `analysis.Buffer` recorded as `parameters:
object[3]` does not tell you what ran, while the full parameter list does.

### ArcPy script integrity

`python.run_arcpy_script` takes a script path, and that file can change after it
runs. The audit record therefore captures the script's SHA-256, byte length, and
last write time at execution time, and archives a content-addressed copy under
`<audit-log-directory>/audit-scripts/<sha256>.py`. Scripts larger than 1 MB are
hashed but not archived. A later edit to the original script leaves the archived
copy intact, so the log can still produce the code that actually ran.

### Limitations

Audit writes fail silently by design so that logging cannot break a bridge
request. Contended appends are retried, but a persistent failure - a full disk or
a permissions problem - drops the record without surfacing an error. The log is
also plain appended JSONL in a user-writable location, so it is not tamper
evident. Treat it as a best-effort record, not a sealed one.

## MCP Client Example

After publishing the server, configure your MCP client with the generated
executable and config path:

```json
{
  "mcpServers": {
    "arcgis-pro": {
      "type": "stdio",
      "command": "C:\\path\\to\\ArcGisProMcpServer.exe",
      "args": [],
      "env": {
        "ARCGIS_PRO_MCP_CONFIG": "C:\\path\\to\\arcgis-pro-mcp.config.json"
      }
    }
  }
}
```

For development, use `.mcp.json` from the repository root.

### Claude Code

Claude Code can register the published server directly:

```powershell
claude mcp add arcgis-pro --scope user `
  --env "ARCGIS_PRO_MCP_CONFIG=C:\path\to\publish\ArcGisProMcpServer\arcgis-pro-mcp.config.json" `
  -- "C:\path\to\publish\ArcGisProMcpServer\ArcGisProMcpServer.exe"
```

Verify with `claude mcp list`, which reports the server as connected once it
responds to an MCP handshake. `pro_ping` works without ArcGIS Pro running, so it
is the quickest end-to-end check.

## Client Compatibility

The server speaks plain MCP over stdio and depends on no vendor SDK, so it works
with any MCP client. One naming detail governs portability:

**Tool and prompt names use underscores, not dots** - `pro_ping`, not `pro.ping`.

Dots are legal in the MCP spec, but Anthropic's API constrains tool names to
`^[a-zA-Z0-9_-]{1,64}$` and rejects dotted names outright. Claude Code also
prefixes MCP tools as `mcp__<server>__<tool>`, and that prefixed form has to fit
inside the same 64-character limit. OpenAI enforces the same character class.
Underscore names therefore satisfy both vendors, whereas dotted names only work
on clients that silently sanitize them.

This applies only to the MCP surface. The named-pipe protocol between the server
and the ArcGIS Pro add-in still uses dotted operation names (`pro.health`,
`map.list`), and so do bridge error codes and the `enabledToolGroups` config
gating. Renaming a tool does not require touching the add-in.

## Agent Prompt

After the MCP server is configured and ArcGIS Pro is open with the add-in
loaded, give your agent a prompt like this:

```text
You have access to the ArcGIS Pro MCP Bridge. Use it to inspect the current
ArcGIS Pro project before making changes. Prefer dry-run operations first for
map, layer, layout, legend, export, geoprocessing, project-save, and ArcPy
script actions. Use stable object IDs returned by the read tools when editing
maps, layers, layouts, legends, and map frames. Preview layouts or active maps
after visual changes, critique the result, and iterate only as needed. Do not
run destructive geoprocessing, overwrite files, save projects, or execute ArcPy
scripts unless I explicitly confirm that action. When writing ArcPy map-making
or layout/export automation, consult docs/arcpy-mapmaking-guide.md first.
```

## Smoke Test

1. Build the solution.
2. Package and register the add-in.
3. Restart ArcGIS Pro.
4. Start or publish the MCP server.
5. Call `pro_ping`; it should return `pong` without needing ArcGIS Pro.
6. Open ArcGIS Pro with the add-in loaded.
7. Call `pro_health`; it should return bridge and project status.
8. Call `project_get_current`, `map_list`, `layer_list`, or `layout_list`.
9. Call `visual_capture_active_view` or `visual_export_layout_preview` to verify
   image artifact output.

## Documentation

- `docs/arcpy-mapmaking-guide.md` - ArcGIS Pro-first ArcPy guidance for agents.

## Safety Model

The bridge is designed for local use with an active ArcGIS Pro session. It
supports dry runs, stable object IDs, configurable allowed roots, confirmation
flags, disabled destructive operations by default, and optional audit logging.

Review tool calls before allowing saves, overwrites, destructive geoprocessing,
or ArcPy script execution.

## License

Free for noncommercial use under the
[PolyForm Noncommercial License 1.0.0](LICENSE). That covers personal use and
evaluation, nonprofits, educational and research institutions, and government
agencies, including state, county, and municipal GIS departments, regardless of
how that work is funded.

Commercial use requires a separate paid license. That includes consultancies
using the bridge on billable client work, producing map or data deliverables you
sell, embedding it in a commercial product or service, and for-profit internal
use. Evaluating it is always free.

See [COMMERCIAL-LICENSE.md](COMMERCIAL-LICENSE.md) for the details and how to
get in touch.
